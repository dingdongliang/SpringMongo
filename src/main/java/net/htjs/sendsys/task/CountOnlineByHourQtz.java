...
/**
 * Description:定时统计每小时在线用户数
 * author  dyenigma
 * date 2016/9/12 16:42
 */
public class CountOnlineByHourQtz extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        String begin = DateUtil.getSpecifiedDay(-1).substring(0, 13) + ":00:00";
        String end = DateUtil.getSpecifiedDay(-1).substring(0, 13) + ":59:59";
        ...
        for (UserRule u : userRules) {
            ...
            counts.setCountTime(DateUtil.getSpecifiedDay(-1).substring(0, 13));
            counts.setOnlineCounts(result.size());
            counts.setProject(u.getProject());
            counts.setCountType("H");
            //将统计结果保存到数据库
