...
My97DP.prototype = {init:function() {
    ...
    if (J && J.loadDate)$ = J; else {
        ...
        if (!L && M.length != _.length) {
            ...
            while ((M = A.exec(C)) !== null) {
                ...
                K += (D == 0) ? "" : (".{" + D + "}");
                D = A.lastIndex;
                switch (M[0]) {case"yyyy":K += "(\\d{4})";break;case"yyy":K += "(\\d{3})";break;default:K += "(\\d\\d?)";break}
            }
            K += ".*$";
        ...
        if (M)for (H = 0; H < _.length; H++) {
            var I = M[H + N];
            if (I)switch (_[H]) {case"y":case"yy":I = pIntDef(I, 0);if (I < 50)I += 2000; else I += 1900;$.y = I;break;case"yyy":$.y = pIntDef(I, 0) + $dp.yearOffset;break;default:$[_[H].slice(-1)] = I;break}
        }
    }
    ...
    if (_ != 0) {
        A = false;
        var $;
        if (_ > 0)$ = this.maxDate; else $ = this.minDate;
        if ($dp.has.sd) {
            $dt.y = $.y;
            $dt.M = $.M;
            $dt.d = $.d
    ...
    for (F = 1,D = A; F < 7; F++) {
        ...
        for (E = 0; E < 7; E++) {
            ...
            if (_.M == B) {
                L = true;
                if (_.compareWith($sdt, "d") == 0)K = "Wselday"; else if (_.compareWith($tdt, "d") == 0)K = "Wtoday"; else K = (($dp.highLineWeekDay && (E == 0 || E == 6)) ? "Wwday" : "Wday");
    ...
    if ($dp.has.sd) {
        if ($dp.has.st)$dp.realFmt = $dp.realFullFmt; else $dp.realFmt = $dp.realDateFmt
    ...
    if (B)C(A); else switch ($dp.errDealMode) {case 0:$cMark = true;if (confirm($lang.errAlertMsg)) {
        A[$dp.elProp] = this.oldValue;
        C(A)
    } else $(A);break;case 1:A[$dp.elProp] = this.oldValue;C(A);break;case 2:$(A);break}
    function C(A) {
    ...
    $ = $ || $dt;
    var E,B,D,A;
    switch (C.charAt(0)) {case"w":A = getDay($);break;case"D":B = [C];A = $lang.aWeekStr[getDay($) + 1];break;case"W":A = getWeek($);break;case"y":B = ["yyyy","yyy","yy","y"];break;default:break}
    B = B || [C + C,C];
    _ = _ || B[0];
    ...
    if ($dp.onpicked)$dp.onpicked.call($dp.el, $dp); else if (this.oldValue != $dp.el[$dp.elProp] && $dp.el.onchange)fireEvent($dp.el, "change")
},initBtn:function() {
    $d.clearI.onclick = function() {
        ...
        if (!$) {
            ...
            $c.setRealValue("");
            hide($dp.dd);
            if ($dp.oncleared)$dp.oncleared.call($dp.el, $dp); else if ($c.oldValue != $dp.el[$dp.elProp] && $dp.el.onchange)fireEvent($dp.el, "change")
        }
    };
    $d.okI.onclick = function() {
        day_Click()
    };
    if ($dp.el[$dp.elProp] == "")$d.okI.value = $lang.okStr; else $d.okI.value = $lang.updateStr;
    ...
    if (E > $)E = $; else if (_ == "m" || _ == "s")C = [0,15,30,45,59,-60,-45,-30,-15,-1]; else for (H = 0; H < $ * 2; H++)C[H] = $dt[_] - $ + 1 + H;
    for (H = G = 0; H < E; H++) {
...
function dealRange(type) {
    ...
    rv = $c.checkRange();
    if (rv == 0)eval(func); else if (rv < 0)_setAll($c.minDate); else if (rv > 0)_setAll($c.maxDate);
    function _setAll($) {
        s_y($.y);
        s_M($.M);
        s_d($.d);
...
function updownEvent($) {
    ...
    switch ($c.currFocus) {case $d.HI:c_H($dt.H + $);break;case $d.mI:c_m($dt.m + $);break;case $d.sI:c_s($dt.s + $);break}
}
